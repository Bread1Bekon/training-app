...
class TokenService:
    ...
    def create_access_token(self, data: dict):
        ...
        self.token_repository.add_access_token(token)
        return token
    ...
    def create_refresh_token(self, data: dict):
        ...
        token = jwt.encode(to_encode, settings.PRIVATE_KEY, algorithm=settings.ALGORITHM)
        self.token_repository.add_refresh_token(token)
        return token

    def token_validation(self, token: str):
        response = self.token_repository.check_token(token)
        if response:
            return True
...
class UserService:
    ...
    async def log_in(self, user_email: str, user_password: str):
        ...
        refresh_token = self.token_service.create_refresh_token(data={"sub": user.id})
